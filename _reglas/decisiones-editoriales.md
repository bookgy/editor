...
- aplicación:
  ...
  - Las reglas generales sustituyen el concepto `entrada principal` por `blog` cuando hablan de la pieza desarrollada.
...
## DEC-2026-09-009 — La carpeta de referencias pasa a llamarse reglas
...
- afecta a:
  ...
  - `_reglas/decisiones-editoriales.md`
- decisión:
  - La carpeta común `_referencias/` pasa a llamarse `_reglas/`.
  - `contexto.md`, `editorial.md`, `guia-redaccion.md` y `decisiones-editoriales.md` conservan su función y contenido; cambia su ubicación canónica.
  - Todas las Skills y la documentación general deben utilizar exclusivamente rutas bajo `_reglas/`.
- aplicación:
  - Migrar los cuatro archivos desde `_referencias/` a `_reglas/`.
  - Actualizar cualquier instrucción que apunte a `_referencias/...` para que apunte a `_reglas/...`.
  - Mantener `_fuentes/` como carpeta separada para la fuente factual del producto.
